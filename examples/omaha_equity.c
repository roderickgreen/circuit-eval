/* Omaha heads-up equity: exact, by enumerating every board, using the
 * library's fused circuit_equity_omaha call.  omaha_equity_manual.c
 * computes the same thing at the plane level (bsapi.h).  Build with
 ...
 * Hole and board masks stay separate: the evaluator applies the
 * two-from-hole, three-from-board rule itself. */
#include <stdio.h>
#include <stdint.h>
...
{
    uint64_t ha[BS_BATCH], hb[BS_BATCH], bd[BS_BATCH];
    ...
    for (int c4 = c3 + 1; c4 < 52; c4++) {
        ...
        if (++n == BS_BATCH) {
            uint64_t w, t;
            circuit_equity_omaha(ha, hb, bd, n, &w, &t);
